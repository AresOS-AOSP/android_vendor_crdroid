...
def LoadOptions(input_file):
  ...
  try:
    ...
    OPTIONS.input_tmp = input_tmp
    info = OPTIONS.info_dict = common.LoadInfoDict(input_tmp)

  if not info or ('dynamic_partition_list' not in info and
                  'super_block_devices' not in info):
    input_tmp = common.UnzipTemp(input_file)
    OPTIONS.input_tmp = input_tmp
    info = OPTIONS.info_dict = common.LoadInfoDict(input_tmp)
  ...
  OPTIONS.dynamic_partition_list = info.get('dynamic_partition_list', '').strip().split()
  OPTIONS.super_device_list = info.get('super_block_devices', '').strip().split()
